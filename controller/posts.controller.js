...
class PostsController {
    ...
    async getPosts(req, res) {
        ...
        if (!user_id) {
            ...
            const users = await db.query(`SELECT id, user_id, posttext as text, likecount FROM posts WHERE user_id = $1 ORDER BY id DESC LIMIT $2 OFFSET $3`, [user_id, size, (current - 1) * (size)]);
    ...
    async updatePost(req, res) {
            ...
            const post = await db.query(`UPDATE posts set user_id = $1, posttext = $2 WHERE user_id = $3 RETURNING *`, [user_id, posttext, id])
